...
def main():
    ...
    for i in os.listdir(args.img_path):
        name = i.split('.')
        img = tf.image.decode_jpeg(tf.read_file(os.path.join(args.img_path,i)), channels=3)
        ...
        print('Inference single image need: {:.5f}'.format(end-start))
        
        msk = decode_labels(preds, num_classes=args.num_classes)
        im = Image.fromarray(msk[0])
        im1 = im.resize((512,512),Image.BICUBIC)
        if not os.path.exists(args.save_dir):
            os.makedirs(args.save_dir)
#        im.save(args.save_dir + 'mask.png')
        im1.save(args.save_dir + name[0]+'_predict.png')
